...
set_of_values = [int(v) for v in np.arange(80, 134, 5)]
...
for alg_name in algorithms:
    ...
    for number_of_drivers in set_of_values:
        algorithm_result[number_of_drivers] = []
        for trial in range(trials):
            ...
            print(
                f"\nAlg: {alg_name}- Parameter: {number_of_drivers} - Trial: {trial + 1} / {trials} - Dataset with {n_requests} requests and {len(data_generator.drivers)} drivers has been loaded.")
            ...
            algorithm_result[number_of_drivers].append(total_info)
