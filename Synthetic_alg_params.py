...
set_of_values = [float(alpha) for alpha in np.arange(0, 1.01, 0.05)]
...
for alg_name in algorithms:
    ...
    algorithm_result = {}
    if alg_name == "MyAlg":
        running_params = set_of_values
        unassigned_tol = 10
    else:
        running_params = [set_of_values[0]]
        unassigned_tol = 50

    for alpha in running_params:
        algorithm_result[alpha] = []
        for trial in range(trials):
            ...
            print(
                f"\nAlg: {alg_name}- Parameter: {alpha} - Trial: {trial + 1} / {trials} - Dataset with {n_requests} requests and {len(data_generator.drivers)} drivers has been loaded.")
            ...
            algorithm_result[alpha].append(total_info)

    with open('results/Synthetic_alg_params/{0}.json'.format(alg_name), 'w') as writer:
