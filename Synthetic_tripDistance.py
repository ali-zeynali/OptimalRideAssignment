...
set_of_values = [int(v) for v in np.arange(1, 34, 5)]
...
for alg_name in algorithms:
    ...
    for avg_trip_distance in set_of_values:
        algorithm_result[avg_trip_distance] = []
        for trial in range(trials):
            ...
            print(
                f"\nAlg: {alg_name}- Parameter: {avg_trip_distance} - Trial: {trial + 1} / {trials} - Dataset with {n_requests} requests and {len(data_generator.drivers)} drivers has been loaded.")
            ...
            algorithm_result[avg_trip_distance].append(total_info)

        with open('results/Synthetic_tripDistance/{0}.json'.format(alg_name), 'w') as writer:
