...
set_of_values = [int(v) for v in np.arange(10, 61, 5)]
...
myAlg = MyAlg(driver_move_range=driver_move_range, distance_options=distance_options,
              max_waiting_list=max_waiting_list, alpha=gamma, V_coeff=V_coeff, lr=0.1)
...
for alg_name in algorithms:
    ...
    for request_intervals in set_of_values:
        algorithm_result[request_intervals] = []
        for trial in range(trials):
            ...
            print(
                f"\nAlg: {alg_name}- Parameter: {request_intervals} - Trial: {trial + 1} / {trials} - Dataset with {n_requests} requests and {len(data_generator.drivers)} drivers has been loaded.")
            ...
            algorithm_result[request_intervals].append(total_info)

    with open('results/Synthetic_reqIntervals/{0}.json'.format(alg_name), 'w') as writer:
